...
$sqlcommand01 = "SELECT * FROM timesheet WHERE empID={$empID}";
	...
	<body>
		...
		padding:30px;">
			...
			style="background:linear-gradient(#800000,#800000);">
				<h2 class="text-center text-warning"><?php echo($names["$empID"]); ?> Work History</h2>
				  ...
				  <tbody>
					<?php
						while($row = $result01->fetch_assoc())
						{
						    ...
						    {
							echo "<td><a href=\"admin-approval-process.php?action={$row['shiftNum']}&back=admin-work-history.php?empID={$empID}\">{$row['approval']}</a></td>";
						    } else { 
